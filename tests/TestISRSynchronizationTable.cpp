...
TEST(BasicSyncTable, SpeedComparisonAnd) {
    ...
    for (int docID = 1; docID <= num_docs; ++docID) {
        int doc_start = (docID - 1) *
                        doc_length;  // each doc has word corpus ranging [0...99], [100...199], etc.
    ...
    mcd_offset = 0;
    PostingList mcd_postingList_equipped_with_sync_table =
        PostingList::NewDeserialize(static_cast<char*>(mcd_base_region2), mcd_offset);
    close(fd2);
    ...
    PostingList::NewSerialize(static_cast<char*>(burgers_base_region), burgers_offset,
                              burgers_postingList);
    ...
    burgers_offset = 0;
    PostingList burgers_postingList_equipped_with_sync_table =
        PostingList::NewDeserialize(static_cast<char*>(burgers_base_region2), burgers_offset);
    close(fd4);
    ...
    std::cout << "It took " << time_taken << " seconds to find all the matching documents"
              << std::endl;
...
TEST(BasicSyncTable, SpeedComparisonPhrase) {
    ...
    // ISRWord::OldSeek AND ISRPhrase::OldCatchUpStragglerISRs took on average 1.6 seconds
    // ISRWord::OldSeek AND ISRPhrase::NewCatchUpStragglerISRs took on average 0.008 seconds
    // ISRWord::NewSeek AND ISRPhrase::NewCatchUpStragglerISRs took on average 0.005 seconds
    // these results make sense
    std::vector<uint32_t> matching_documents;
